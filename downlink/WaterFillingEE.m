%% Water Filling algorithm for Energy Efficiency

function [PowerOpt, WaterLevel] = WaterFillingEE(ChannelState, PowerTotal, Band, lambda, Performance)
...
X = max((Band / (lambda * Performance + v) - 1./ChannelState), 0);
...
X = max((Band / (lambda * Performance + v_av) - 1./ChannelState), 0);
